...
const OUTPUT_PATH = join("data", "predictions", "group-stage-monte-carlo-v1.json");
...
export async function runMonteCarloGroupStagePrediction(): Promise<void> {
  ...
  warnings.push("Fair-play/team-conduct data is not collected in Phase 1; simulations skip that tiebreaker and use FIFA rank if all simulated scoreline tiebreakers remain tied.");
  warnings.push("Host/venue advantage is not applied because fixture locations and home/away venue assignments are not in Phase 1 model input.");
  ...
  for (const team of teamInputs) accumulators.set(teamKey(team.team), createAccumulator(team));

  for (let iteration = 0; iteration < options.iterations; iteration += 1) {
    const simulation = simulateTournament(teamInputs, groups, rules, rng, modelParameters);
  ...
  const output = {
    ...
    predictionId: "group-stage-monte-carlo-v1",
    predictionType: "group_stage_monte_carlo",
    generatedAt,
    ...
    simulation: {
      ...
      monteCarloIterations: options.iterations,
    },
    method: {
      type: "poisson_monte_carlo_group_stage",
      languageModelUsedForTeamOrdering: false,
      matchModel:
        "Poisson goals. Lambda = collected recent-form base goals * sqrt(team attack index * opponent defensive-vulnerability index) * exp((team quality score - opponent quality score) * qualityMultiplierScale), clamped to [lambdaMin, lambdaMax].",
      modelParameters,
  ...
  console.log(`Monte Carlo group-stage prediction: wrote ${OUTPUT_PATH}`);
...
function simulateTournament(teamInputs: TeamInput[], groups: GroupDefinition[], rules: Rules, rng: () => number, modelParameters: ModelParameters) {
  const byTeam = new Map(teamInputs.map((team) => [teamKey(team.team), team]));
  const groupResults = groups.map((group) => simulateGroup(group, byTeam, rules, rng, modelParameters));
...
function simulateGroup(group: GroupDefinition, byTeam: Map<string, TeamInput>, rules: Rules, rng: () => number, modelParameters: ModelParameters) {
  ...
  for (let i = 0; i < teams.length; i += 1) {
    for (let j = i + 1; j < teams.length; j += 1) {
      ...
      const match = { teamA: teamA.team, teamB: teamB.team, goalsA, goalsB };
      matches.push(match);
      updateRows(rows, match, rules);
    }
  }

  const rankedRows = rankGroupRows([...rows.values()], matches, rules);
  return { group: group.group, rankedRows, matches };
}
...
function rankGroupRows(rows: TableRow[], matches: MatchResult[], rules: Rules): TableRow[] {
  return breakTies(rows, matches, [
    { name: "points", direction: "desc", score: (row, tiedRows) => (tiedRows.length === rows.length ? row.points : row.points) },
    { name: "h2hPoints", direction: "desc", score: (row, tiedRows) => headToHead(row, tiedRows, matches, rules).points },
    { name: "h2hGoalDifference", direction: "desc", score: (row, tiedRows) => headToHead(row, tiedRows, matches, rules).goalDifference },
    { name: "h2hGoalsFor", direction: "desc", score: (row, tiedRows) => headToHead(row, tiedRows, matches, rules).goalsFor },
    { name: "goalDifference", direction: "desc", score: (row) => row.goalDifference },
    { name: "goalsFor", direction: "desc", score: (row) => row.goalsFor },
    { name: "fifaRank", direction: "asc", score: (row) => row.fifaRank },
  ]);
}
...
type TieCriterion = {
  name: string;
  direction: "asc" | "desc";
  score: (row: TableRow, tiedRows: TableRow[]) => number;
};

function breakTies(rows: TableRow[], matches: MatchResult[], criteria: TieCriterion[], criterionIndex = 0): TableRow[] {
  ...
  for (const row of rows) {
    ...
    const tiedRows = groups.get(score) ?? [];
    tiedRows.push(row);
    groups.set(score, tiedRows);
  }

  const orderedScores = [...groups.keys()].sort((a, b) => (criterion.direction === "desc" ? b - a : a - b));
  return orderedScores.flatMap((score) => breakTies(groups.get(score) ?? [], matches, criteria, criterionIndex + 1));
}
...
function buildInputStats(teamStrength: TeamStrengthRow[], recentForm: RecentFormRow[]) {
  ...
  const goalsFor = recentForm.map((row) => row.goalsForPerMatch);
  const goalsAgainst = recentForm.map((row) => row.goalsAgainstPerMatch);
  ...
  return {
    ...
    averageGoalsForPerMatch: average(goalsFor),
    averageGoalsAgainstPerMatch: average(goalsAgainst),
  };
}
...
function compareTeamSummaries(a: TeamSummary, b: TeamSummary): number {
  return (
    ...
    b.advanceProbability - a.advanceProbability ||
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runMonteCarloGroupStagePrediction().catch((error: unknown) => {
